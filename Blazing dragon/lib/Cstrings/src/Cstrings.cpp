...
string_t* string_make_upper(string_t* s, size_t length);
string_t* string_make_lower(string_t* s, size_t length);
...
string_t* string_make_upper(string_t* s, size_t length)
{
    ...
    {
        s->string[i] = ( (s->string[i]) - 20);   /* eg. z in hex is 7A - 20 = 5A -> Z */
    }
    return s;
}
...
string_t* string_make_lower(string_t* s, size_t length)
{
    for(int i = 0; i < length; i++)
    {
        s->string[i] = (20 + (s->string[i]));   /* eg. A in hex is 41 + 20 = 61 -> a */
    }
    return s;
}
